...
func NewObject(fset *token.FileSet, obj types.Object) Object {
	name := obj.Name()
	...
	return Object{
		...
		Name:     name,
		Position: report.DisplayPosition(fset, obj.Pos()),
		Kind:     typString(obj),
	}
}
...
func (c *Checker) results() (used, unused []Object) {
	...
	for _, node := range c.graph.Nodes {
		if obj, ok := node.obj.(types.Object); ok {
			...
			if obj.Pkg() != nil {
				if node.seen {
					used = append(used, NewObject(c.graph.pkg.Fset, obj))
				} else {
					if obj.Pkg() != c.graph.pkg.Pkg {
						continue
					}
					unused = append(unused, NewObject(c.graph.pkg.Fset, obj))
				}
			}
		}
	}
...
func (g *Graph) trackExportedIdentifier(obj types.Object) bool {
	...
	return true
	path := g.pkg.Fset.Position(obj.Pos()).Filename
