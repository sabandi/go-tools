...
// XXX get rid of binary export data: https://github.com/golang/go/issues/37312

// XXX are we accidentally returning the root node in Run?

// XXX restore live stats functionality
...
func (r Result) Unused() (unused.Result, error) {
	if r.Failed {
		panic("Unused called on failed Result")
	}
	if r.unused == "" {
		// this package was only a dependency
		return unused.Result{}, nil
	}
	var res unused.Result
	f, err := os.Open(r.unused)
	if err != nil {
		return unused.Result{}, fmt.Errorf("failed loading unused: %w", err)
	}
	defer f.Close()
...
func (r *subrunner) do(act action) error {
	...
	if !a.factsOnly {
		// OPT(dh): is there any reason to store these in different
		// files? wouldn't it be simpler to store all of these in a
		// single file?
		f2, _, err2 = r.cache.GetFile(cache.Subkey(a.hash, "directives"))
		f3, _, err3 = r.cache.GetFile(cache.Subkey(a.hash, "diagnostics"))
		f4, _, err3 = r.cache.GetFile(cache.Subkey(a.hash, "unused"))
	}
	...
	if err1 != nil || err2 != nil || err3 != nil || err4 != nil {
		...
		for _, diag := range result.diags {
			d := Diagnostic{
				Pos:      report.DisplayPosition(result.lpkg.Fset, diag.Pos),
			...
			for _, sugg := range diag.SuggestedFixes {
				...
				for _, edit := range sugg.TextEdits {
					s.TextEdits = append(s.TextEdits, TextEdit{
						Pos:     report.DisplayPosition(result.lpkg.Fset, edit.Pos),
			...
			for _, rel := range diag.Related {
				d.Related = append(d.Related, RelatedInformation{
					Pos:     report.DisplayPosition(result.lpkg.Fset, rel.Pos),
		...
		f4, err = r.writeCacheGob(a, "unused", result.unused)
		if err != nil {
			return err
		}
	}
			...
			for _, cg := range cgs {
				for _, c := range cg.List {
					...
					d := Directive{
						Command:           cmd,
						Arguments:         args,
						DirectivePosition: report.DisplayPosition(pkg.Fset, c.Pos()),
						NodePosition:      report.DisplayPosition(pkg.Fset, node.Pos()),
					}
					dirs = append(dirs, d)
				}
			}
